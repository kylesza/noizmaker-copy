...
func onReady(s *discordgo.Session, event *discordgo.Ready) {
	...
	s.UpdateStatus(0, "fire beats son.")
}
...
func onGuildCreate(s *discordgo.Session, event *discordgo.GuildCreate) {
	...
	for _, channel := range event.Guild.Channels {
		if channel.ID == event.Guild.ID {
			s.ChannelMessageSend(channel.ID, "**NOIZMAKER BOT READY. TYPE `!AIRHORN` WHILE IN A VOICE CHANNEL TO ACTIVATE**")
			return
		}
	}
}
...
func main() {
	...
	// We're running!
	log.Info("noizmaker is ready to funk it up.")
